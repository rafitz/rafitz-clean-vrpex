...
AddEventHandler("queue:playerConnecting",function(source,ids,name,setKickReason,deferrals)
	...
	if ids ~= nil and #ids > 0 then
		deferrals.update("[BASE] Carregando identidades.")
		local user_id = vRP.getUserIdByIdentifiers(ids)
		if user_id then
			deferrals.update("[BASE] Carregando banimentos.")
			if not vRP.isBanned(user_id) then
				deferrals.update("[BASE] Carregando whitelist.")
				if vRP.isWhitelisted(user_id) then
					if vRP.rusers[user_id] == nil then
						deferrals.update("[BASE] Carregando banco de dados.")
					...
					deferrals.done("[BASE] Nosso servidor tem Whitelist! [ Entre no nosso Discord: https://discord.gg/ ] [ ID: "..user_id.." ]")
					TriggerEvent("queue:playerConnectingRemoveQueues",ids)
				end
			else
				deferrals.done("[BASE] Você foi banido da cidade. [ ID: "..user_id.." ]")

				TriggerEvent("queue:playerConnectingRemoveQueues",ids)
			end
		else
			deferrals.done("[BASE] Ocorreu um problema de identificação.")
			TriggerEvent("queue:playerConnectingRemoveQueues",ids)
		end
	else
		deferrals.done("[BASE] Ocorreu um problema de identidade.")
		TriggerEvent("queue:playerConnectingRemoveQueues",ids)
	end
end)
