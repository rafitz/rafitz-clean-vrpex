function WPrompt(){
	...
	this.divParent.classList.add('main-wprompt');
...
WPrompt.prototype.open = function(title,text){
	document.querySelector('.main-wprompt').classList.add('open');
	this.close();
...
WPrompt.prototype.close = function(){
	if(this.opened){
		...
		this.opened = false;
		this.div.style.display = "none";
		document.querySelector('.main-wprompt').classList.remove('open');
	}
}